	...
	switch (proto) {
		...
		case SEPOL_PROTO_IP4:
		{
			...
			if (inet_ntop(AF_INET, &addr, addr_str, 
				...
				ERR(handle, "could not expand IPv4 address to string: %s",
					strerror(errno));
				return STATUS_ERR;
			}
			break;
		...
		case SEPOL_PROTO_IP6:
		{
			...
			if (inet_ntop(AF_INET6, &addr, addr_str, 
				...
				ERR(handle, "could not expand IPv6 address to string: %s",
					strerror(errno));
				return STATUS_ERR;
			}
			break;
...
static int node_alloc_addr_string(
	sepol_handle_t* handle,
	int proto,
	char** addr) {
...
int sepol_node_key_create(
	...
	if (node_alloc_addr(handle, proto, &tmp_key->addr, &tmp_key->addr_sz) < 0)
		goto err;
	if (node_parse_addr(handle, addr, proto, tmp_key->addr) < 0)
		goto err;

	if (node_alloc_addr(handle, proto, &tmp_key->mask, &tmp_key->mask_sz) < 0)
		goto err;
...
int sepol_node_compare(
	const sepol_node_t* node,
	const sepol_node_key_t* key) {

	int rc1, rc2;

	if ((node->addr_sz < key->addr_sz) ||
...
const char* sepol_node_get_proto_str(
	int proto) {
...
/* Create */
int sepol_node_create(
	sepol_handle_t* handle,
	sepol_node_t** node) {

	sepol_node_t* tmp_node = 
		(sepol_node_t*) malloc(sizeof(sepol_node_t));
...
/* Context */
sepol_context_t* sepol_node_get_con(
	const sepol_node_t* node) {
